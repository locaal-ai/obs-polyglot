	  ...
	  model_info(model_info)
{
	...
	model_name_label->setText(QString::fromStdString(model_info.name));
...
void ModelDownloader::download_finished(const ModelInfo &info)
{
	this->model_info.localPath = info.localPath;
	this->model_info.localSpmPath = info.localSpmPath;

	// Call the callback with the path to the downloaded model
	this->download_finished_callback(0, this->model_info);
	// Close the dialog
	this->close();
}
...
ModelDownloadWorker::ModelDownloadWorker(const ModelInfo &model_info_)
{
	this->model_info = model_info_;
}

std::string ModelDownloadWorker::download_file(CURL *curl, const std::string &url,
					       const std::string &path)
{
	// Check if the file already exists
	if (std::filesystem::exists(path)) {
		obs_log(LOG_INFO, "File already exists: %s", path.c_str());
		return "";
	}

	FILE *fp = fopen(path.c_str(), "wb");
	if (fp == nullptr) {
		obs_log(LOG_ERROR, "Failed to open file %s.", path.c_str());
	...
	curl_easy_setopt(curl, CURLOPT_XFERINFOFUNCTION, ModelDownloadWorker::progress_callback);
	curl_easy_setopt(curl, CURLOPT_XFERINFODATA, this);
	// Follow redirects
	curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
	CURLcode res = curl_easy_perform(curl);
	if (res != CURLE_OK) {
		obs_log(LOG_ERROR, "Failed to download model %s.", this->model_info.name.c_str());
...
void ModelDownloadWorker::download_model()
{
	...
	char *model_save_path_str = obs_module_get_config_path(obs_current_module(), "models");
	...
	if (curl) {
		const std::string model_file_save_base_path =
			model_save_path + "/" + this->model_info.name;
		// Create if doesn't exist
		if (!std::filesystem::exists(model_file_save_base_path)) {
			if (!std::filesystem::create_directories(model_file_save_base_path)) {
				obs_log(LOG_ERROR, "Failed to create model folder: %s",
					model_file_save_base_path.c_str());
		...
		for (const std::string &url : this->model_info.urls) {
			// extract filename from URL that has querystring
			std::string filename = url.substr(url.find_last_of("/\\") + 1);
			filename = filename.substr(0, filename.find("?"));

			const std::string model_file_save_path =
				model_file_save_base_path + "/" + filename;
			std::string error = download_file(curl, url, model_file_save_path);
			if (!error.empty()) {
				emit download_error(error);
				return;
			}
		}
		obs_log(LOG_INFO, "Model downloaded to %s", model_file_save_base_path.c_str());

		// Download the sentencepiece model
		const std::string sp_model_file_save_path =
			model_save_path + "/" + this->model_info.name + ".sp.model";
		std::string error = download_file(curl, model_info.spmUrl, sp_model_file_save_path);
		if (!error.empty()) {
			emit download_error(error);
			return;
		}
		obs_log(LOG_INFO, "SPM downloaded to %s", sp_model_file_save_path.c_str());

		// Save the model path to the model_info
		this->model_info.localPath = model_file_save_base_path;
		this->model_info.localSpmPath = sp_model_file_save_path;

		emit download_finished(this->model_info);
	} else {
